...
from Crossvalidation import fit_FFNN_checkpoint
...
if not pseudo:
    model = '{}-NoPseudo'.format(model)
    nam = '{}{}'.format(nam,'-NoPseudo')

if (number > 1):
    model = '{}-Ensemble'.format(model)
    nam = nam = '{}{}'.format(nam,'-Ensemble')


run_classes = ['MAPK1','MTOR']
        ...
        for fold_set in fold_sets:
            for fold in folds:
                ...
                if len(glob(save_fp)) > 0: 
                    ...
                    continue
                args = (data, fold_columns, clss_column, class_columns, batch_size, num_batches, num_epochs, verbose,
                        num_layers, num_outputs, learning_rates, loss_fn, tune, check_dir, fold_set, fold, number, pseudo)
