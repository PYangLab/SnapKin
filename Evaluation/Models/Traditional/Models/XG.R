...
fit_XG = function(input, n_trees=500, smote=FALSE, no_pseudo=FALSE,num_models=1) {
  # Output
  ...
  for (fold_num in 1:num_repeats) {
    ...
    for (i in fold_numbers) {
      ...
      for (n.models in 1:num_models) {
        model[[n.models]] = boost_tree(mode = "classification", trees=n_trees) %>%
      ...
      num_neg_preds = nrow(X_negatives)
