...
def snapkin(train_fp, test_fp, save_dir, save_test_name='predictions.csv', verbose=1,
            num_layers=3, num_snaphots=10, num_epochs=150, batch_size=32, learning_rate=0.01, 
    ...
    save_dirs = ['{}{}'.format(os.path.join(save_dir,'Saved_Models','Model_'),mod_nam) for mod_nam in range(num_snaphots)]
...
def run_snapkin(args_dict):
    ...
    for arg in ARGS:
        if arg not in keys:
            ...
            return 
    # Run Snapkin 
    snapkin(train_fp=args_dict['train_fp'],
            ...
            num_snaphots=int(args_dict['num_snapshots']),
